...
EVENT_HREF = re.compile(r'^/auctions/[^?#]+-\d+/?$')
PAGE_HREF = re.compile(r'/page-(\d+)$')
LOT_HREF = re.compile(r'^/auctions/[^/]+/[^/?#]+-\d+/?$')
...
def date_from_text(text):
    # Prefer an explicit weekday/date line over titles such as "28 & 29 July 2026".
...
def discover_events():
    ...
    for page in range(1, 40):
        ...
        for a in soup.find_all('a', href=True):
            ...
            found += 1
        # The archive currently exposes a finite numbered pager. Stop only after
        # we have passed the populated pages, so a transient empty page cannot
        # truncate page 1 discovery.
...
def max_pages(event_url, html):
    soup = BeautifulSoup(html, 'html.parser')
    pages = [1]
    for a in soup.find_all('a', href=True):
        m = PAGE_HREF.search((a.get('href') or '').rstrip('/'))
        if m:
...
def nearest_lot_block(anchor):
    ...
    for parent in anchor.parents:
        ...
        if re.search(r'\bLot\s+[\w.-]+\b', txt, re.I) and len(txt) < 5000:
            chosen = parent
            # Prefer a self-contained card with result/price text.
...
def parse_page(event, page_url, html):
    ...
    rows = []
    seen = set()
    event_path = '/' + event['url'].split('/', 3)[3].strip('/') + '/'
    for a in soup.find_all('a', href=True):
        href = a.get('href') or ''
        if not LOT_HREF.match(href):
            continue
        full = urljoin(BASE, href)
        if full in seen:
            continue
...
def crawl_event(event):
    ...
    pages = max_pages(event['url'], first_html)
    collected = []
    page_shapes = []
    ...
    # Completeness is based on exhausting the site's own paginator, not the
    # archive 'Offered' metric (which can differ because withdrawn/section lots
    # remain represented in the catalogue).
...
def main(batch_size=4):
    ...
    global_progress = load_json(PROGRESS_FILE, {'sources': {}})
    old = (global_progress.get('sources') or {}).get('Savills') or {}
    ...
    remaining = max(0, len(events) - len(completed))
    source_progress = {
        'status': 'CAUGHT UP' if remaining == 0 and not failures else ('BACKFILLING' if processed or completed else 'DISCOVERED'),
